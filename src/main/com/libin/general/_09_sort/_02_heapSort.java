...
class heapSort {
	...
	public void heapAdjust(int[] tmpNum, int parent, int length) {
		...
		while (lChild < length) {
			...
			if (rChild < length && tmpNum[lChild] < tmpNum[rChild]) {
				lChild++;
			}
			//当父节点的值大于孩子节点的时候，直接结束
			if (tmp >= tmpNum[lChild]) {
				break;
			}
			tmpNum[parent] = tmpNum[lChild];
	...
	public void heapScheduler(int[] tmpNum) {
		...
		for (int i = (tmpNum.length - 1) / 2; i >= 0; i--) {
			heapAdjust(tmpNum, i, tmpNum.length);
		}
		sortPtint();
		//进行排序
		int tmp = 0;
